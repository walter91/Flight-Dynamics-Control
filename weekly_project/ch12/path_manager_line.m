...
function out = path_manager_line(in,P,start_of_simulation)
  ...
  c      = [0; 0; 0]; % only for fillets
  rho    = 0; % only for fillets
  lambda = 0; % only for fillets
  n = (q_min1+q)/norm(q_min1+q);
  
  out = [flag; Va_d; r; q_min1; c; rho; lambda; state; flag_need_new_waypoints];

  % determine if next waypoint path has been reached, and rotate ptrs if
  % necessary
  r = w;
  plane_cross = dot(p-r,n);
  if dot(p-r,n) >= 0,
