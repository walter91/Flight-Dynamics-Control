...
% should I be using atan or atan2 to calculate angles below?
  ...
  if ell<2*R,
    ...
    L1 = ell + R*mod(2*pi+mod(theta-pi/2,2*pi)-mod(chis-pi/2,2*pi),2*pi) + R*mod(2*pi+mod(chie-pi/2,2*pi)-mod(theta-pi/2,2*pi),2*pi);
    % compute L2 (R-S-L)
    ell = norm(cle-crs);
    ...
    if isreal(theta2)==0, 
      ...
      L2 = sqrt(ell^2-4*R^2) + R*mod(2*pi+mod(theta2,2*pi)-mod(chis-pi/2,2*pi),2*pi) + R*mod(2*pi+mod(theta2+pi,2*pi)-mod(chie+pi/2,2*pi),2*pi);
    end
    % compute L3 (L-S-R)
    ell = norm(cre-cls);
    ...
    if isreal(theta2)==0,
      ...
      L3 = sqrt(ell^2-4*R^2) + R*mod(2*pi+mod(chis+pi/2,2*pi)-mod(theta+theta2,2*pi),2*pi) + R*mod(2*pi+mod(chie-pi/2,2*pi)-mod(theta+theta2-pi,2*pi),2*pi);
    end
    % compute L4 (L-S-L)
    ell = norm(cls-cle);
    theta = -(atan2(cle(1)-cls(1),cle(2)-cls(2)) - pi/2);
    L4 = ell + R*mod(2*pi+mod(chis+pi/2,2*pi)-mod(theta+pi/2,2*pi),2*pi) + R*mod(2*pi+mod(theta+pi/2,2*pi)-mod(chie+pi/2,2*pi),2*pi);
