...
function out = path_follow(in,P)
  ...
  switch flag,
      case 1, % follow straight line path specified by r and q
          ei_p = [pn; pe; h] - r_path;
          ki = [0; 0; h];
          n = cross(q_path,ki)/mag3(cross(q_path, ki));
      ...
      case 2, % follow orbit specified by c, rho, lam
          ...
          phi_low = atan2(pe-c_orbit(2), pn-c_orbit(1));
          if(phi_low - chi < -pi)
              phi_low = phi_low + 2*pi;
          end
          if(phi_low - chi > pi)
              phi_low = phi_low - 2*pi;
          end
          
          chi_c = phi_low + lam_orbit*((pi/2) + atan(P.korbit*((d - rho_orbit)/rho_orbit)));
          if lam_orbit == 1
            phi_ff = 0.3;
          end
          if lam_orbit == -1
